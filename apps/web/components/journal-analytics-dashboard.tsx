...
function BucketTable({ title, rows }: { title: string; rows: JournalAnalyticsBucket[] }) {
  return (
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.key}>
                  ...
                  <td>{row.win_rate != null ? `${row.win_rate.toFixed(0)}%` : '—'}</td>
                  <td
                    className={
...
export function JournalAnalyticsDashboard({ analytics }: { analytics: JournalAnalytics }) {
  return (
    <div>
      <div className="card">
        ...
        <div className="kpis" style={{ marginTop: 12, marginBottom: 0 }}>
          ...
          <div className="kpi">
            ...
            <div className="kpi-value">
              {analytics.win_rate != null ? `${analytics.win_rate.toFixed(0)}%` : '—'}
            </div>
          </div>
        </div>
      ...
      <BucketTable title="Performance by signal strength" rows={analytics.by_signal_label} />

      <BucketTable title="Performance by news source" rows={analytics.by_news_source} />
