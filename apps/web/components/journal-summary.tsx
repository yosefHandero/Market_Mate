...
export function JournalSummary({ entries }: { entries: JournalEntry[] }) {
  ...
  const tookWithPnl = entries.filter(
    (entry) => entry.decision === "took" && entry.pnl_pct != null,
  );

  const avgTookPnl = tookWithPnl.length
    ? tookWithPnl.reduce((sum, entry) => sum + (entry.pnl_pct ?? 0), 0) /
      tookWithPnl.length
    : null;

  const tookWins = tookWithPnl.filter(
    (entry) => (entry.pnl_pct ?? 0) > 0,
  ).length;

  const tookWinRate = tookWithPnl.length
    ? (tookWins / tookWithPnl.length) * 100
    : null;
  ...
  const tookSkippedNewsWithPnl = entries.filter(
    (entry) =>
      entry.decision === "took" &&
      entry.news_source === "skipped" &&
      entry.pnl_pct != null,
  );

  const avgMarketauxPnl = tookMarketauxWithPnl.length
    ? tookMarketauxWithPnl.reduce(
        (sum, entry) => sum + (entry.pnl_pct ?? 0),
        0,
  ...
  return (
    <div className="history-item" style={{ marginBottom: 10, padding: 12 }}>
      ...
      <div className="small muted" style={{ lineHeight: 1.8, marginTop: 6 }}>
        Avg took P/L:{" "}
        {avgTookPnl != null
          ? `${avgTookPnl >= 0 ? "+" : ""}${avgTookPnl.toFixed(2)}%`
          : "—"}
        {" • "}
      ...
      <div className="small muted" style={{ lineHeight: 1.8, marginTop: 6 }}>
        Strong avg:{" "}
        {avgStrongPnl != null
          ? `${avgStrongPnl >= 0 ? "+" : ""}${avgStrongPnl.toFixed(2)}%`
          : "—"}
        {" • "}
        Watch avg:{" "}
        {avgWatchPnl != null
          ? `${avgWatchPnl >= 0 ? "+" : ""}${avgWatchPnl.toFixed(2)}%`
          : "—"}
      </div>
      ...
      <div className="small muted" style={{ lineHeight: 1.8, marginTop: 6 }}>
        ...
        Cache avg:{" "}
        {avgCachePnl != null
          ? `${avgCachePnl >= 0 ? "+" : ""}${avgCachePnl.toFixed(2)}%`
          : "—"}
        {" • "}
        Skipped-news avg:{" "}
